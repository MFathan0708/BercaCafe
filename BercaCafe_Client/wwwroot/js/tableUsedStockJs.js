...
$(document).ready(function () {
    ...
    table = $('#tblUsedStockReport').DataTable({
        "paging": true,
        "autoWidth": false,
        "responsive": true,
        "ajax": {
            "url": "https://localhost:44331/api/Stocks/StockTerpakai/?fromDate=" + splitDate[0].toString().trim().replaceAll('/', '-') + "&thruDate=" + splitDate[1].toString().trim().replaceAll('/', '-'),
            "type": "GET",
            "dataType": "json",
            "dataSrc": "result",
            async: true,
...
function searchFromDate() {
    $('#tblUsedStockReport').DataTable().clear().draw();
    splitDate = $('#dateFilter').val().toString().split('-');
    table = $('#tblUsedStockReport').DataTable({
        "paging": true,
        "autoWidth": false,
